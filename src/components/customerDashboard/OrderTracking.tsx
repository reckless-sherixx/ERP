...
export default function OrderTracking() {
    ...
    const handleTrackOrder = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`/api/v1/customerDashboard/trackOrder/track-order?orderNumber=${encodeURIComponent(orderNumber)}&email=${encodeURIComponent(email)}`);
    ...
    const handleDesignAction = async (action: 'approve' | 'revision', submissionId: string) => {
        setActionInProgress(true);
        try {
            const response = await fetch(`/api/v1/customerDashboard/trackOrder/design-action`, {
                method: 'POST',
                headers: {
